...
@section('container')
    ...
    <div class="col-lg-8">
        <form method="POST" action="/dashboard/categories/{{ $category->slug }}" class="mb-5" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                <label for="image" class="form-label">Category Image <small>(Optional)</small></label>
                <input type="hidden" name="oldImage" value="{{ $category->image }}">
                @if ($category->image)
                    <img src="{{ asset('storage/' . $category->image) }}" class="img-fluid img-preview mb-3 col-sm-5 d-block">
                @else
